...
public class StatementBuilder {
    ...
    private LineNumberTable lnt_attribute;
    ...
    public StatementBuilder(Code code_attribute, int start, int end,
            String descriptor, boolean isStatic){
        this(code_attribute, start, end, new ArrayList<Integer>());
        this.vars.addAll(lvt_attribute.getNamedVars(descriptor, isStatic));
    }
    
    public StatementBuilder(Code code_attribute, int start,
            int end, List<Integer> vars) {
        ...
        this.lvt_attribute = code_attribute.getLocalVariableTableAttribute();
        this.lnt_attribute = code_attribute.getLineNumberTableAttribute();
...
//        lastIndex = this.variables.size() - 1;
            ...
            while (i < end) {
                ...
                switch (opcode) {
                    ...
                    case Opcode.IF_ICMPLE:
                        System.err.println("consumeIF_CMP");
                        i = consumeIF_CMP(i, OperationNode.Type.GT);
                        break;

                    case Opcode.IF_ACMPEQ:
                    case Opcode.IF_ICMPEQ:
                        System.err.println("consumeIF_CMP");
                        i = consumeIF_CMP(i, OperationNode.Type.NE);
                        break;
                        
                    case Opcode.IF_ACMPNE:
                    case Opcode.IF_ICMPNE:
                        System.err.println("consumeIF_CMP");
                        i = consumeIF_CMP(i, OperationNode.Type.EQ);
                        break;
                        
                    case Opcode.IF_ICMPGT:
                        System.err.println("consumeIF_CMP");
                        i = consumeIF_CMP(i, OperationNode.Type.LE);
                        break;
                        
                    case Opcode.IF_ICMPGE:
                        System.err.println("consumeIF_CMP");
                        i = consumeIF_CMP(i, OperationNode.Type.LT);
                        break;
                        
                    case Opcode.IF_ICMPLT:
                        System.err.println("consumeIF_CMP");
                        i = consumeIF_CMP(i, OperationNode.Type.GE);
                        break;
    ...
    private int consumeIF_CMP(int pos, OperationNode.Type cmp) throws Exception {
...
//        System.err.println("branch= " + branch);
    ...
    private int consumePUTSTATIC(int pos) {
        ...
        statements.add(new AssignNode(field, value, AssignNode.Type.NONFIRST, this.lnt_attribute.getLine(pos)));
    ...
    private int consumeINCREMENT(int pos) {
        ...
        String[] type_name = lvt_attribute.getVar(index);
        ...
        statements.add(new AssignNode(variable,
                ...
                AssignNode.Type.NONFIRST, this.lnt_attribute.getLine(pos)));
    ...
    private int consumeSTORETOARRAY(int pos) {
        ...
        statements.add(new AssignNode(new ArrayRefNode(ref, new String[]{count.getName()}), 
                value, AssignNode.Type.NONFIRST, this.lnt_attribute.getLine(pos)));
        return pos + 1;
    }
...
//        FMIref_Info ref_info = pool().getFMIref_Info(index);
    ...
    private int consumeSTOREVARIABLE(int pos, int index) {
        ...
        AssignNode assign = new AssignNode(variable, value, atype, 
                this.lnt_attribute.getLine(pos));
