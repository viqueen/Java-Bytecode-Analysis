        ...
        if (!method.isStaticInit()) {
            ...
            if (lvt_attribute != null) {
                ...
                if (numOfParameters > 0) {
                    String[] variable = lvt_attribute.getVar(lvt_attribute.THIS_INDEX + 1);
                    ...
                    for (int i = 2; i <= numOfParameters; i++) {
                        builder.append(", ");
                        variable = lvt_attribute.getVar(lvt_attribute.THIS_INDEX + i);
                        builder.append(variable[0]);
                        builder.append(" ");
                        builder.append(variable[1]);
                    }
        ...
        if (method.isAbstract()) {
            ...
            if (lvt_attribute != null) {
                StatementBuilder st_builder = new StatementBuilder(code_attribute, 0,
                        code_attribute.getCode().length,
                        method.getDescriptor(), AccessFlags.isStatic(method.getAccess_flags()));
                try {
